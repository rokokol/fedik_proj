...
namespace fedik_proj {
    ...
    int constexpr N = 500000 + 1;

    int constexpr S = 5;
    int constexpr B[5] = {1, 0, 0, 4, 1};

    std::vector<std::pair<double, double> > rungekut(double (*f)(double, double, double),
                                                     double (*g)(double, double, double), double x_0, double y_0,
                                                     double h = H, double n = N) {
        ...
        for (int i = 0; i < N - 1; i++) {
            ...
            double k_3_x = f(t_n + h / 3, x_n + h * k_1_x / 6 + h * k_2_x / 6, y_n + h * k_1_x / 6 + h * k_2_x / 6);
            double k_4_x = f(t_n + h / 2, x_n + h / 8 * k_1_x + 3. / 8 * h * k_3_x,
                             y_n + h / 8 * k_1_x + 3. / 8 * h * k_3_x);
            double k_5_x = f(t_n + h, x_n + h / 2 * k_1_x - 3. / 2 * h * k_3_x + 2 * h * k_4_x,
            ...
            double k_3_y = g(t_n + h / 3, x_n + h * k_1_y / 6 + h * k_2_y / 6, y_n + h * k_1_y / 6 + h * k_2_y / 6);
            double k_4_y = g(t_n + h / 2, x_n + h / 8 * k_1_y + 3. / 8 * h * k_3_y,
                             y_n + h / 8 * k_1_y + 3. / 8 * h * k_3_y);
            double k_5_y = g(t_n + h, x_n + h / 2 * k_1_y - 3. / 2 * h * k_3_y + 2 * h * k_4_y,
    ...
    auto real_test_x = [](double t)-> double {
        return cos(t) / sqrt(1 + exp(2 * t));
    };

    auto real_test_y = [](double t)-> double {
    ...
    int main() {
        ...
        for (int i = 0; i < N; i += 100000) {
            ...
            if (absx > max_autox) max_autox = absx;
            if (absy > max_autoy) max_autoy = absy;

            printf("%-6.3f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f\n",
                   t,
                   res[i].first, rx,
                   res[i].second, ry,
                   absx / fabs(rx),
